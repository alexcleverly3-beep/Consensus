...
function createRecurrenceDashboardStore(db, {
  ...
  const recurringWallets = db.prepare(`
    WITH recurring AS (
      SELECT wallet_address,
        ...
        SUM(CASE WHEN user_discord_evidence = 1 THEN 1 ELSE 0 END) AS discord_pick_tokens,
    ...
    LIMIT ?
  `);
  const recurringWalletCount = db.prepare(`
  ...
  const cancelQueuedToken = db.prepare(`
    ...
    SET status = 'cancelled', priority = 0, priority_queued_at = NULL,
        user_discord_priority = 0, last_error = NULL
  ...
  return {
    ...
    wallets({ minDistinctTokens = 3, limit = 250 } = {}) {
      ...
      return recurringWallets.all(min, bounded).map((row) => ({
        ...
        discordPickTokens: num(row.discord_pick_tokens),
    ...
    walletCount({ minDistinctTokens = 3 } = {}) {
      const min = Math.max(2, Math.min(100, Math.floor(Number(minDistinctTokens) || 3)));
      return num(recurringWalletCount.get(min)?.total);
    },
...
function renderPrivateDashboard(stats, wallets, {
  ...
  const qualifyingWalletTotal = Math.max(wallets.length, num(walletTotal));
  const walletRows = wallets.length ? wallets.map((wallet, index) => {
    ...
    return `<tr class="${wallet.checked ? "wallet-checked" : ""}"><td>${index + 1}</td><td class="mono"><a href="https://solscan.io/account/${encodeURIComponent(wallet.walletAddress)}" target="_blank" rel="noreferrer">${escapeHtml(wallet.walletAddress)}</a>${checkControl}</td><td><strong>${wallet.distinctTokens}</strong></td><td>${wallet.totalAppearances}</td><td>${wallet.top10Tokens}</td><td>${wallet.top25Tokens}</td><td>${wallet.discordPickTokens || 0}</td><td>${wallet.bestRank}</td><td>${wallet.averageBestRank == null ? "—" : escapeHtml(wallet.averageBestRank)}</td><td>${escapeHtml(flags)}</td><td>${escapeHtml(formatTime(wallet.lastSeenAt))}</td></tr>`;
  }).join("") : `<tr><td colspan="11">No wallets have reached ${minDistinctTokens}+ distinct token appearances yet.</td></tr>`;
...
.wallet-total{margin-top:12px;padding:11px 13px;border:1px solid var(--border-soft);border-radius:9px;background:#0b131c;color:var(--muted);font-size:12px;text-align:right}.wallet-total strong{color:var(--text);font-variant-numeric:tabular-nums}
