...
function renderPhase2WalletLab(items, csrfToken = "", notice = "", live = {}) {
  ...
  const usageKinds = status.heliusUsageTodayByKind || {};
  const liveCreditsToday = usageKinds["webhook-delivery"]?.credits || 0;
  const recoveryCreditsToday = usageKinds.rpc?.credits || 0;
  const managementCreditsToday = usageKinds["webhook-management"]?.credits || 0;
  const dailyAllowance = status.dailyCreditAllowance || Math.floor((status.monthlyCreditBudget || 800000) / 30);
  const liveCards = [
    ...
    ["Daily credit allowance", dailyAllowance], ["Live / recovery today", `${liveCreditsToday} / ${recoveryCreditsToday}`],
    ["Recovery RPC", status.recoveryRpcSource || "helius"], ["Recovery calls today", `${status.recoveryCallsToday || 0} / ${status.recoveryDailyCallBudget || 3000}`],
    ["Live wallet budget", `${status.liveBudget?.effectiveLimit ?? config.trackedWalletLimit ?? 40} / ${status.liveBudget?.requestedLimit ?? config.trackedWalletLimit ?? 100}`],
    ["Live credit target/day", status.liveBudget?.liveDailyCreditBudget || dailyAllowance],
    ...
    ["Missed events recovered", status.recoveredEvents || 0], ["Helius credits this month", `${status.estimatedHeliusCredits || 0} / ${status.monthlyCreditBudget || 800000}`],
  ].map(([label, value]) => `<div class="live-card"><span>${esc(label)}</span><strong>${esc(value)}</strong></div>`).join("");
  const nearSignals = (live.nearSignals || []).length
    ? live.nearSignals.map((signal) => `<tr><td><a href="https://dexscreener.com/solana/${encodeURIComponent(signal.tokenMint)}" target="_blank" rel="noreferrer">${esc(signal.tokenMint)}</a></td><td>${esc(signal.walletCount)}</td><td>${esc(signal.totalPoints)}</td></tr>`).join("")
    : '<tr><td colspan="3" class="empty">No live wallet accumulations in the current window.</td></tr>';
  const providerNote = provider.last_error && provider.status !== "active"
    ...
    : "";
  const creditNote = Number(status.estimatedHeliusCreditsToday || 0) > dailyAllowance
    ? `<p class="credit-note"><strong>Credit warning:</strong> Today is above the roughly ${esc(dailyAllowance)}-credit daily pace for the monthly allowance. Breakdown: ${esc(liveCreditsToday)} live events, ${esc(recoveryCreditsToday)} recovery checks, ${esc(managementCreditsToday)} webhook management.</p>`
    : "";
  const recoveryNote = status.recoveryConfigurationError
    ? `<p class="provider-note"><strong>Recovery RPC:</strong> ${esc(status.recoveryConfigurationError)}</p>`
    : "";
  const entryRule = gate.minDistinctTokens
    ? `<p class="settings"><strong>Phase 2 entry:</strong> ${esc(gate.minDistinctTokens)}+ distinct Phase 1 tokens, ${esc(gate.minTop10Tokens)}+ top-10 appearances, and average rank ${esc(gate.maxAverageRank)} or better. Creator and insider wallets are allowed. Tracks up to the top ${esc(config.trackedWalletLimit || 100)} qualifying wallets, temporarily fewer when needed to protect the Helius credit budget.</p>`
    : "";
  const controls = `<details class="controls"><summary>Phase 2 controls</summary><form method="post" action="/actions/phase2/settings"><input type="hidden" name="csrf" value="${esc(csrfToken)}"><div class="control-grid"><label>Minimum distinct tokens<input type="number" name="minDistinctTokens" min="3" max="100" value="${esc(gate.minDistinctTokens || 10)}" required></label><label>Minimum top-10s<input type="number" name="minTop10Tokens" min="0" max="100" value="${esc(gate.minTop10Tokens ?? 1)}" required></label><label>Average rank this or better<input type="number" name="maxAverageRank" min="1" max="100" value="${esc(gate.maxAverageRank || 50)}" required></label><label>Wallets tracked<input type="number" name="trackedWalletLimit" min="1" max="100" value="${esc(config.trackedWalletLimit || 100)}" required></label><label>Wallets per alert<input type="number" name="minDistinctWallets" min="2" max="20" value="${esc(config.minDistinctWallets || 3)}" required></label><label>Points per alert<input type="number" name="pointsThreshold" min="2" max="100" value="${esc(config.pointsThreshold || 6)}" required></label><label>Alert window (minutes)<input type="number" name="signalWindowMinutes" min="5" max="1440" value="${esc(Math.round((config.signalWindowMs || 3600000)/60000))}" required></label></div><div class="control-actions"><p>GMGN checks up to five top-100 candidates every 30 minutes and refreshes them weekly. Profitable wallets with 20+ results can earn up to +10 reputation, including a small +1/+2 for 6h/24h average holds. A sub-35% win rate only receives a soft -10 when there are 30+ results and realised profit is not positive; no wallet is blacklisted as a bot. Helius only counts confirmed quote-to-token buys.</p><button type="submit">Save and refresh</button></div></form></details>`;
  const livePanel = `<section class="live"><div class="live-head"><div><h2>Live wallet signals</h2><p>Separate Phase 2 monitoring. Phase 1 scanning continues unchanged.</p></div><form method="post" action="/actions/phase2/test-discord"><input type="hidden" name="csrf" value="${esc(csrfToken)}"><button type="submit">Send Discord test</button></form></div><div class="live-grid">${liveCards}</div>${providerNote}${recoveryNote}${creditNote}<p class="settings">Alert at ${esc(config.minDistinctWallets || 3)}+ wallets and ${esc(config.pointsThreshold || 6)}+ points within ${esc(Math.round((config.signalWindowMs || 3600000)/60000))} minutes.</p>${entryRule}${controls}<details><summary>Near-threshold tokens</summary><table><thead><tr><th>Token</th><th>Wallets</th><th>Points</th></tr></thead><tbody>${nearSignals}</tbody></table></details></section>`;
  return `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Consensus Phase 2 Lab</title><style>:root{color-scheme:dark;font-family:Inter,system-ui,sans-serif;background:#080d14;color:#edf4fa}*{box-sizing:border-box}body{margin:0}main{max-width:1120px;margin:auto;padding:32px 20px}a{color:#69b5ff}h1{margin:0;font-size:30px}h2{margin:0}.sub{color:#8fa2b7;margin:8px 0 22px}.notice{background:#10271b;border:1px solid #285f41;padding:10px 12px;border-radius:10px;margin-bottom:16px}.live{background:#0e1822;border:1px solid #274057;border-radius:14px;padding:18px;margin-bottom:22px}.live-head{display:flex;align-items:center;justify-content:space-between;gap:16px}.live-head p,.settings{color:#8fa2b7;margin:6px 0 0}.provider-note,.credit-note{color:#ffd68a;background:#261d0d;border:1px solid #654a20;border-radius:9px;padding:10px 12px;margin:0 0 12px}.credit-note{color:#ffb7b7;background:#2b1114;border-color:#74323a}.live-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin:16px 0}.live-card{background:#091019;border:1px solid #223344;border-radius:10px;padding:12px}.live-card span{display:block;color:#8fa2b7;font-size:12px}.live-card strong{display:block;font-size:20px;margin-top:4px}.live details{margin-top:14px}.live table{width:100%;border-collapse:collapse;margin-top:10px}.live th,.live td{text-align:left;padding:8px;border-bottom:1px solid #223344;font-size:12px}.controls{background:#091019;border:1px solid #223344;border-radius:10px;padding:12px}.controls summary{cursor:pointer;font-weight:700}.control-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-top:14px}.control-grid label{color:#8fa2b7;font-size:12px}.control-grid input{display:block;width:100%;margin-top:5px;background:#0e1822;color:#fff;border:1px solid #34465a;border-radius:8px;padding:9px}.control-actions{display:flex;align-items:center;justify-content:space-between;gap:14px;margin-top:12px}.control-actions p{color:#8fa2b7;font-size:12px;margin:0}.analyze{display:flex;gap:10px;background:#111923;border:1px solid #253243;padding:16px;border-radius:14px;margin-bottom:22px}.analyze input{flex:1;background:#091019;color:#fff;border:1px solid #34465a;border-radius:9px;padding:12px}.analyze button,form button{background:#18304a;color:#fff;border:1px solid #355675;border-radius:8px;padding:8px 11px;cursor:pointer}.wallet{display:flex;gap:18px;border:1px solid #263443;background:#101720;border-radius:14px;padding:18px;margin:12px 0}.score{font-size:38px;font-weight:800;min-width:100px}.score span{font-size:13px;color:#899bad}.body{flex:1}.addr{font-family:ui-monospace,monospace}.meta{color:#97a9bb;margin:7px 0 10px;font-size:13px}.metrics{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:12px}.metrics span{background:#0b121a;border:1px solid #243240;border-radius:999px;padding:5px 8px;font-size:12px}.sel{border-color:#4bbf79!important}.bad{color:#ffadb5}.sel.bad{border-color:#d45c67!important}.empty{color:#8fa2b7;padding:20px;text-align:center}@media(max-width:800px){.live-grid,.control-grid{grid-template-columns:repeat(2,1fr)}}@media(max-width:650px){.analyze,.wallet,.live-head,.control-actions{flex-direction:column;align-items:stretch}.score{min-width:0}}</style></head><body><main><h1>Phase 2 — Wallet Lab</h1><p class="sub">Manual calibration workspace. Scores are provisional research scores, not trusted-wallet promotion.</p>${notice ? `<div class="notice">${esc(notice)}</div>` : ""}${livePanel}<form class="analyze" method="post" action="/actions/phase2/analyze"><input type="hidden" name="csrf" value="${esc(csrfToken)}"><input name="wallet" maxlength="44" placeholder="Paste Solana wallet address" required><button type="submit">Analyse wallet</button></form>${cards}</main></body></html>`;
}
